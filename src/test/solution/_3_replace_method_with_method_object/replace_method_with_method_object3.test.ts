...
import { ReplaceMethodWithMethodObject3 } from "../../../main/solution/_3_replace_method_with_method_object/_3/replace_method_with_method_object3";

test("delta값은 50으로 하고, gamma 메소드에 인자값을 100(입력값), 3(수량), 500(년수) 입력하면 349860을 반환한다", () => {
  // given
  const delta = 50;
  const replaceMethodWithMethodObject = new ReplaceMethodWithMethodObject3(delta);
  // when
